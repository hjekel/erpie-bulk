...
const CACHE_TTL_NULL_MS = 10 * 60 * 1000;   // 10 minutes for null/empty results
...
async function searchBrave(query) {
  ...
  // Check cache (null results expire faster)
  const cached = cache.get(query);
  if (cached) {
    const ttl = cached.result ? CACHE_TTL_MS : CACHE_TTL_NULL_MS;
  ...
  try {
    ...
    // Cache: successful results 1h, null results 10min
...
/**
 * Search Brave for B2C refurbished prices of a device model
 * Identical to searchBrave but uses a separate cache key prefix "b2c:"
 * @param {string} query - Search query
 * @returns {Promise<{prices: number[], source: string, snippet: string} | null>}
 */
async function searchBraveB2C(query) {
  const apiKey = process.env.BRAVE_API_KEY;
  if (!apiKey) {
    console.warn('[web-search] BRAVE_API_KEY not set — skipping live search');
    return null;
  ...
  // Check cache with b2c prefix (null results expire faster)
  const cacheKey = 'b2c:' + query;
  ...
  try {
    ...
    // Cache: successful results 1h, null results 10min
    cache.set(cacheKey, { result: resultObj, timestamp: Date.now() });
...
/**
 * Clear expired cache entries (respects null TTL)
 */
function cleanCache() {
  const now = Date.now();
  for (const [key, val] of cache) {
    const ttl = val.result ? CACHE_TTL_MS : CACHE_TTL_NULL_MS;
...
module.exports = { searchBrave, searchBraveB2C, extractPrices };
