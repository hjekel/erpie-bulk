...
const APPLE_SILICON = {
  ...
  'macbook pro 13 m1': 370,
  'macbook pro m1': 370,         // fallback: any Pro M1 without size → 13" price
  'macbook air m1': 350,
  'macbook air m2': 800,
  'macbook air m3': 1000,
};
...
function calculatePrice(input) {
  ...
  // ── Apple Silicon ────────────────────────────────────────────────────────────
  // Keys are ordered longest-first so "macbook pro 14 m1 pro" matches before "macbook pro m1"
  for (const [key, price] of Object.entries(APPLE_SILICON)) {
    if (normModel.includes(key)) {
      ...
      reasoning.push(`Apple Silicon direct price: €${price} (${key})`);
  ...
  const isAppleSilicon = Object.keys(APPLE_SILICON).some(k => normModel.includes(k));
