...
function calculatePrice(input) {
  ...
  for (const { pat, price, label } of APPLE_SILICON) {
    if (pat.test(appleMatchStr)) {
      const gradeMult   = APPLE_SILICON_GRADE[grade] ?? APPLE_SILICON_GRADE['A'];
  ...
  // 6. NOTEBOOK FACTOR — B2B resale discount for notebooks/laptops
  // Apple Silicon: NO factor (×1.00)
  // Desktops: NO factor (×1.00)
  // All other notebooks: ×0.65 (conservative, per Joep/TOOLS.md)
  const isAppleSilicon = APPLE_SILICON.some(({ pat }) => pat.test(normModel + ' ' + (rawCpu || '').toLowerCase()));
  const isDesktop = /optiplex|thinkcentre|prodesk|elitedesk|desktop|mini pc/i.test(normModel);
  let notebookFactor = 1.00;
  if (!isAppleSilicon && !isDesktop) {
    notebookFactor = 0.65;
    const before = advised;
    advised = Math.round(advised * notebookFactor);
    reasoning.push(`Notebook factor: ×0.65 (€${before} → €${advised})`);
  }
