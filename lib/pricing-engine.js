...
const BASE_PRICES = {
  ...
  'Gen9': 160,  'Gen8': 100,  'Gen7': 63,   'Gen6': 40,   'Gen5': 20,
  'Gen4': 10
};

// ─── WATCH CAPS ──────────────────────────────────────────────────────────────
// Joep 8-apr-2026: Gen8+ = GO, no caps. Gen7 = WATCH, base = €63.
const WATCH_CAPS = {};
...
// ─── GRADE MULTIPLIERS ───────────────────────────────────────────────────────
// Joep 8-apr-2026: Grade A = Grade B = 1.00 ("helemaal niet doen")
// Only C and below get discounted
const GRADE_MULT = {
...
// ─── APPLE SILICON DIRECT PRICES (Grade A base) ─────────────────────────────
// Grade multipliers for Apple Silicon: A=1.00, B=0.90, C=0.75
const APPLE_SILICON_GRADE = { 'A': 1.00, 'B': 0.90, 'C': 0.75, 'D': 0.50 };
// Regex patterns — order matters: most specific first
const APPLE_SILICON = [
  { pat: /macbook\s*pro.*14.*m1\s*pro/i,  price: 650, label: 'MacBook Pro 14 M1 Pro' },
  { pat: /macbook\s*pro.*16.*m1\s*pro/i,  price: 500, label: 'MacBook Pro 16 M1 Pro' },
  { pat: /macbook\s*pro.*m1/i,            price: 352, label: 'MacBook Pro M1' },
  { pat: /macbook\s*air.*m3/i,            price: 1000, label: 'MacBook Air M3' },
  { pat: /macbook\s*air.*m2/i,            price: 800, label: 'MacBook Air M2' },
  { pat: /macbook\s*air.*m1/i,            price: 345, label: 'MacBook Air M1' },
];
...
const MODEL_BASE_PRICE = {
  ...
  // HP EliteBook 850 15.6" (premium over 14" 840 models) — market data Mar 2026
  'elitebook 850 g7': 195, 'elitebook 850 g8': 240, 'elitebook 850 g5': 110, 'elitebook 850 g6': 110,
  ...
  // Lenovo ThinkPad T14 (trades at discount vs HP EliteBook 840) — market data Mar 2026
  't14 gen 1': 160, 't14 gen 3': 250,
  // t14 gen 2 override removed (Joep 8-apr-2026): Gen11 default €210 is correct
  't14s gen 1': 165, 't14s gen 2': 195, 't14s gen 2i': 195, 't14s gen 3': 255,
};
...
// ─── CLASSIFY STATUS ──────────────────────────────────────────────────────────
// Joep 8-apr-2026: Gen8+ = GO, Gen7 = WATCH (grens), Gen6- = NO-GO
...
function calculatePrice(input) {
  ...
  // 0GB RAM/SSD from Blancco = "not reported" → use baseline defaults, not penalties
  const parsedRam = parseRam(rawRam);
  const parsedSsd = parseSsd(rawSsd);
  const ramGb  = snapRam(parsedRam > 0 ? parsedRam : 8);   // 0 → default 8GB baseline
  const ssdGb  = snapSsd(parsedSsd > 0 ? parsedSsd : 256); // 0 → default 256GB baseline
  ...
  // ── Apple Silicon ────────────────────────────────────────────────────────────
  // CPU field often contains "M1"/"M2"/"M3" while model has "MacBook Air 13" 2020"
  // Combine both so the regex can match across fields
  const appleMatchStr = normModel + ' ' + (rawCpu || '').toLowerCase();
  for (const { pat, price, label } of APPLE_SILICON) {
    if (pat.test(appleMatchStr)) {
      // Joep 8-apr-2026: no grade multiplier for Apple Silicon ("helemaal niet doen")
      const regionMult  = REGION_ADJ[region] || 1.00;
      // Joep: 16GB = "tientje meer" → +€12
      const appleRamAdj = ramGb >= 16 ? 12 : 0;
      const advised     = Math.round((price + appleRamAdj) * regionMult);
      const low         = Math.round(advised * 0.85);
      const high        = Math.round(advised * 1.15);
      reasoning.push(`Apple Silicon direct price: €${price} (${label})`);
      if (appleRamAdj) reasoning.push(`16GB RAM: +€${appleRamAdj}`);
      reasoning.push(`Grade ignored (Apple Silicon), Region ${region} ×${regionMult.toFixed(2)}`);
      return {
  ...
  const GEN8_CAPS = [
    ...
    { pat: /thinkpad\s*t480\b/i, cap: 110, label: 'ThinkPad T480 lot (Joep 8-apr: €100-120)' },
    ...
    // Latitude 7480 cap removed — Gen7 base €63 is correct
  ...
  // 6. NOTEBOOK FACTOR — REMOVED per Joep 8-apr-2026
  // Base prices are already calibrated on actual resale data.
  // The ×0.65 factor was a double correction that made all prices ~30% too low.
...
function analyzeDevices(devices, region = 'EU') {
  ...
  return {
    ...
    summary: {
      ...
      recommendation: (() => {
        if (totalValue <= 0) return 'No pricing data available.';
        const goQty = goList.reduce((s, r) => s + (r.qty || 1), 0);
        const goPct = total ? goQty / total : 0;
        if (goPct >= 1.0) return 'Strong portfolio — all assets profitable. Recommend competitive bid.';
        if (goPct > 0.7) return 'Strong portfolio — majority GO assets. Recommend competitive bid.';
        if (goPct >= 0.5) return 'Mixed portfolio — evaluate WATCH assets carefully.';
        return 'Weak portfolio — significant risk. Consider cherry-picking GO assets only.';
      })(),
    },
  };
}
...
module.exports = { calculatePrice, analyzeDevices, normaliseModel, calculateConfidence };
