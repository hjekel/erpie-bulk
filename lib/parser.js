...
function parseZonesInventory(wb) {
  ...
  for (const name of wb.SheetNames) {
    ...
    for (let i = headerIdx + 1; i < rows.length; i++) {
      ...
      const grade = colMap.grade >= 0 ? cellStr(row[colMap.grade]) : '';
      const key = `${brand}|${model}|${(grade || '').toUpperCase()}`;
      if (!groups[key]) {
...
function extractQuantity(s) {
  ...
  // Pre-clean: strip bracket artifacts from AI normalisation output
  // "20x] Dell..." -> "20x Dell...", "[20x Dell...]" -> "20x Dell..."
  str = str.replace(/^\[?\s*(\d+)\s*[xX\u00d7]\s*\]\s*/, '$1x ');
...
function parseTextInput(text) {
  ...
  // ── Deduplicate by model + grade ──
  const merged = [];
  for (const d of devices) {
    const existing = merged.find(m => m.model.toLowerCase() === d.model.toLowerCase() && (m.grade || 'B').toUpperCase() === (d.grade || 'B').toUpperCase());
    if (existing) {
