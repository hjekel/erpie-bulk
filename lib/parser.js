...
function extractGrade(s) {
  ...
  // "Grade X" / "Klasse X" / "Condition X" prefixed
  const prefixed = str.match(/(?:grade|klasse|condition|conditie|zustand|état|etat|grado|nota)\s*[:=]?\s*([A-D]\d?)\b/i);
...
function extractQuantity(s) {
  ...
  const patterns = [
    // "qty: 60" / "quantity: 60" / "aantal: 60" / "anzahl: 60" / "quantité: 60"
    ...
    // "60 stuks" / "60 stück" / "60 pcs" / "60 pieces" / "60 units" / "60 st" / "60 adet" at end
    { re: /[·\-–—]?\s*(\d+)\s*(?:stuks?|st[üu]ck|pcs|pieces?|units?|exemplaren|unidades|unità|sztuk|adet|st)\s*$/i, pos: 'end' },
    // "60 stuks/pcs/units" at start
    { re: /^(\d+)\s*(?:stuks?|st[üu]ck|pcs|pieces?|units?|exemplaren|unidades|sztuk|adet)\s+/i, pos: 'start' },
    // "(60)" anywhere — quantity in parens
    { re: /\((\d+)\)/, pos: 'remove' },
    // "× 60" / "x 60" at end
    { re: /\s*[xX×]\s*(\d+)\s*$/, pos: 'end' },
...
function parseTextInput(text) {
  const EMAIL_JUNK = /^(summarize this email|inbox|re:|fwd:|from:|to me|to:|subject:|sent:|cc:|planbit|caas\s*[-–]?\s*$|feb |mar |jan |apr |may |jun |jul |aug |sep |oct |nov |dec |http|analyseer|target|geef|caps|qwertzu|lot-factor|totaal:|total:|zusammenfassung|résumé)/i;
  ...
  for (const line of lines) {
    ...
    // Extract quantity using unified function
    ...
    if (parts.length >= 2) {
      ...
      // Split on spec boundaries: CPU, resolution, RAM inline
    ...
    // Parse specs from specsPart and/or full line
    const specsStr = specsPart || cleaned;
    const fullStr = cleaned;

    const ramSsdMatch = specsStr.match(/(\d+)\s*(?:GB)?\s*\/\s*(\d+)\s*(?:GB)?/);
    ...
    // Detect brand from model if not already present
