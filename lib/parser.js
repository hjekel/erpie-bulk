...
function parseBlanccoDetail(wb) {
  const sheetName = wb.SheetNames.find(n =>
    n.toLowerCase().includes('customer inventory') || n.toLowerCase().includes('inventory and details')
  );
  if (!sheetName) return null;

  const rows = XLSX.utils.sheet_to_json(wb.Sheets[sheetName], { header: 1, defval: '' });

  // Find "Original Input" marker or detail header row
  let headerIdx = -1;
  for (let i = 0; i < rows.length; i++) {
    const cells = rows[i].map(v => cellStr(v).toLowerCase());
    // Look for the detail header: has "mfg" and "model (blancco)" or "serial number"
...
function detectFormat(wb) {
  ...
  // Check for PlanBit ARS Blancco detail format
  const hasArsDetail = wb.SheetNames.some(n =>
    n.toLowerCase().includes('customer inventory and details') ||
    n.toLowerCase().includes('inventory and details')
  );
  if (hasArsDetail) {
    // Check if it has the "Original Input" / Blancco detail section
    const detailSheet = wb.SheetNames.find(n => n.toLowerCase().includes('inventory and details'));
    if (detailSheet) {
      const rows = XLSX.utils.sheet_to_json(wb.Sheets[detailSheet], { header: 1, defval: '' });
      for (let i = 0; i < rows.length; i++) {
        const cells = rows[i].map(v => cellStr(v).toLowerCase());
        if (cells.some(c => c === 'mfg') && cells.some(c => c.includes('blancco') || c === 'serial number')) {
...
function parseTextInput(text) {
  ...
  const EMAIL_JUNK = /^(summarize this email|inbox|re:|fwd:|from:|to me|to:|subject:|sent:|cc:|planbit|caas\s*[-–]?\s*$|feb |mar |jan |apr |may |jun |jul |aug |sep |oct |nov |dec |http|analyseer|target|geef|caps|qwertzu|lot-factor|totaal:|total:|zusammenfassung|résumé|beste|hierbij|met vriendelijke|mvg|groet)/i;
